...
do while not eof()
  ...
  seek ncsp_koo.ncsp
  if released
    do while trim(ncsp_en.code_est)<>trim(ncsp_koo.code) and trim(ncsp_en.code) = trim(ncsp_koo.ncsp) and not eof()
      skip
    enddo
    if released and trim(ncsp_en.code_est) = trim(ncsp_koo.code)
