...
SET SAFETY OFF 
SELECT ncsp_luv
COPY TO ..\text\eng_ncsp_luv.txt SDF 
SELECT ncsp_ryh
COPY TO ..\text\eng_ncsp_ryh.txt SDF 
SELECT ncsp_ala
COPY TO ..\text\eng_ncsp_ala.txt SDF 
SELECT ncsp_koo
...
SET SAFETY OFF 
COPY TO ncsp_eng.sdf TYPE SDF
SET SAFETY ON
define popup suodval from 5,5 font  max_foty,  max_fosi prompt files like s_*.dbf
on selection popup suodval do ..\englanti\suodval with prompt()
