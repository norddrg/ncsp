...
define window koodit from ala_y,3 to max_y,luv_x font  max_foty,  max_fosi Title 'NCSP-local'
...
do while not eof()
  ...
  seek ncsp_koo.ncsp
  if released
    do while trim(ncsp_en.code_eng)<>trim(ncsp_koo.code) and trim(ncsp_en.code) = trim(ncsp_koo.ncsp) and not eof()
      skip
    enddo
    if released and trim(ncsp_en.code_eng) = trim(ncsp_koo.code)
