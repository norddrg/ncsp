...
lc_ncsp=ncsp
on error do ..\islanti\apualo
release window apu
on error

activate window valikko
clear
@ 2,2 say 'NCSP Islannin versio'
...
on key label f10 do ..\islanti\aakjarj

ON KEY LABEL f9 
ON KEY LABEL ctrl+f9

on key label F5 do ..\islanti\aakvent
...
on key label F6 do ..\islanti\aaklop
on key label ctrl+f6 do ..\islanti\aakkos
on key label alt+F6 do ..\islanti\aakscand

on key label alt+f do ..\islanti\aakhaku

on key label f9 do ..\islanti\aakfilt
on key label F12 do ..\islanti\aakedit
on key label alt+F12 do ..\islanti\sanasiir
on key label Ctrl+F12 do ..\islanti\sananay


DEFINE WINDOW aakkos FROM 0,30 TO max_y,max_x FONT  max_foty,  max_fosi
activate window aakkos

on error do ..\islanti\sanavaus
select sanat
on error
set order to sana

on error do ..\islanti\hakavaus
select ncsp_hak
on error
