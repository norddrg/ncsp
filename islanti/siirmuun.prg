...
procedure siirmuun
  ...
  copy to siirto\ncsp
  copy to siirto\ncsp_db4;
  fields code, type, text, text_2, short, usedate, chadate, reldate, released, subst, suod.old, ncsp, suod.price;
  type foxplus
  set safety on
  use siirto\ncsp
  select ncsp_luv
  ...
  do WHILE not eof()
    if substr(code,1,1)>oldkoodi
      ...
      insert into ncsp (code, type, text, text_2, english, usedate, chadate, reldate, released, note, note_2, ncsp);
      values (ncsp_luv.code, 'L', ncsp_luv.text, ncsp_luv.text_2, ncsp_luv.english, ncsp_luv.usedate,; 
      ncsp_luv.chadate, ncsp_luv.reldate, ncsp_luv.released, ncsp_luv.note, ncsp_luv.note_2, ncsp_luv.ncsp)
      select ncsp_luv
    ...
    if substr(code,1,2)>oldkoodi
      ...
      if found()
        ...
        ncsp_ryh.chadate, ncsp_ryh.reldate, ncsp_ryh.released, ncsp_ryh.note, ncsp_ryh.note_2, ncsp_ryh.ncsp)
        select ncsp_ryh
        if len(trim(note))>0 or len(trim(note_2))>0
          ? '#OHJE_S  '
          ? trim(note)
    ...
    if substr(code,1,3)>oldkoodi
      ...
      if found()
        ...
        ncsp_ala.chadate, ncsp_ala.reldate, ncsp_ala.released, ncsp_ala.note, ncsp_ala.note_2, ncsp_ala.ncsp)
        select ncsp_ala
        if len(trim(note))>0 or len(trim(note_2))>0
          ? '#OHJE_E  '
          ? trim(note)
    ...
    ? '#LYHENNE '
    ?? trim(short) at 12
    insert into ncsp (code, type, text, text_2, english, short, usedate, chadate, reldate, released, subst, ncsp, note, note_2, old_csp, ncsp);
    ...
    ncsp_koo.note, ncsp_koo.note_2, suod.old, ncsp_koo.ncsp)
    if len(trim(note))>0 or len(trim(note_2))>0
      ? '#OHJE_E  '
      ? trim(note)
      ? '#OHJE_I '
