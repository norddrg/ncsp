...
do while not eof()
  ...
  do while ncsp_en.code=ncsp_koo.code and not eof()
    if ncsp_en.code_ice=ncsp_koo.code and not released
      lc_n=lc_n+1
    endif
    skip
  enddo
  ...
  if lc_n=1
    seek ncsp_koo.code
    do while ncsp_en.code=ncsp_koo.code and not eof()
      if ncsp_en.code_ice=ncsp_koo.code and (not released or not nc)
         lc_find=.t.
    ...
    if ncsp_koo.ncsp<>ncsp_koo.code and ncsp_koo.ncsp<>' ' and not lc_find
      seek ncsp_koo.ncsp
      do while ncsp_en.code=ncsp_koo.ncsp and not eof()
        if nc and not released and code_ice<>' '
          lc_tark=.t.
          seek ncsp_koo.code
          do while ncsp_en.code=ncsp_koo.code and not eof()
            if not released and not nc and code_ice=ncsp_koo.code
              if ncsp=' '
          ...
          enddo
          if lc_tark
            insert into ncsp_en (chadate, usedate, released, code, ncsp, english, code_ice, nc);
  ...
  if not lc_find and not ncsp_koo.released
    seek ncsp_koo.code
    if found() and ncsp_en.code_ice=ncsp_koo.code
      do ..\islanti\simcheck
    else
      if ncsp_koo.ncsp<>'  '
        seek ncsp_koo.ncsp
  ...
  if lc_find
    ...
    if ncsp_koo.released
      ...
      if code_swe=' ' and code_den=' ' and code_nor=' ' and code_fin=' ' and code_est=' ' and code_eng=' ' and not nc
        replace released with .f.
      endif
    endif
  endif
  if not lc_find and not ncsp_koo.released
    insert into ncsp_en (chadate, usedate, released, code, english, code_ice, nc);
    values (ncsp_koo.chadate, ncsp_koo.usedate, .f., ncsp_koo.code, ncsp_koo.text, ncsp_koo.code, .f.)
    if ncsp_en.english=' '
      replace ncsp_en.english with ncsp_koo.text_2
    endif
  endif
...
procedure simcheck
      lc_loop=.t.
      on key label f12 do ..\islanti\en_fin
      on key label f11 do ..\islanti\fin_en
      on key label f10 do ..\islanti\no_match
      do while lc_loop
        do ..\islanti\simnayt
        select ncsp_koo
        ...
        ncsp_koo.reldate:6:r, ncsp_koo.text_2:50 nowait save nomodify
        select ncsp_en
*        set order to code
        browse window codes fields ncsp_en.released:2, ncsp_en.nc:2, ncsp_en.code:7, ncsp_en.ncsp:7, ncsp_en.english:50,;
        ncsp_en.code_ice:7, ncsp_en.chadate:6 :r, ncsp_en.usedate:6:r, ;
...
procedure en_fin
if not ncsp_en.nc
  if ncsp_en.code_ice=ncsp_koo.code and ncsp_koo.ncsp=ncsp_en.ncsp and released
    replace released with .f.
  else
...
procedure fin_en
if not ncsp_en.nc
  if ncsp_en.code_ice=ncsp_koo.code and released
    replace released with .f.
    lc_find=.t.
    do ..\islanti\simnayt
    return
...
do while ncsp_en.code=ncsp_koo.code and not eof()
  ...
  skip
enddo
if not (ncsp_en.code=ncsp_koo.code and ncsp_en.code_ice=ncsp_koo.code and (ncsp_en.ncsp=ncsp_koo.code or ncsp_en.nc)) and not ncsp_koo.released
    insert into ncsp_en (ncsp, code, code_ice, nc);
...
if ncsp_en.code_ice=ncsp_koo.code
  replace ncsp_en.code_ice with ' '
  if not ncsp and code_swe=' ' and code_den=' ' and code_nor=' ' and code_est=' ' and code_eng=' '
...
? 'Accept with [Ctrl][W]'
?
? 'NCSP Icelandic version' STYLE 'B'
