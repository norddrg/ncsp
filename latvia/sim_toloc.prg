...
lc_loop=.t.
if ncsp_en.ncsp=' '
  if ncsp_en.code_lat=ncsp_koo.code and ncsp_koo.ncsp=ncsp_en.ncsp and released
  ...
  seek ncsp_koo.code
  do while trim(code_lat)=trim(ncsp_koo.code) and len(trim(code))>0
    if ncsp<>ncsp_koo.ncsp
      replace code_lat with ' '
      if code_den=' ' and code_swe=' 'and code_nor=' ' and code_est=' ' and code_fin=' ' and code_eng=' ' and ncsp=' ' AND code_ice=' '
         delete
      endif
      seek ncsp_koo.code
      loop
