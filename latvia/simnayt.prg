...
? 'F7  = update the screen with current Latvian code'
? 'F8  = update the screen with current common code'
? 'F12 = accept mapping from NCSP+ to NCSP Latvian version'
? 'F11 = accept mapping from NCSP Latvian version to NCSP+'
...
if ncsp_koo.ncsp=' '
  ...
  if ncsp_en.code_lat <> ncsp_koo.code 
    if ncsp_en.code_lat<>' ' 
      select ncsp_koo
...
set filter to reldate=ctod(space(8))
set order to code
browse window codes fields ncsp_en.ncsp:9, ncsp_en.nc:2, ncsp_en.code_lat:8, ncsp_en.text:50,;
ncsp_en.chadate:6 :r, ncsp_en.usedate:6:r, ;
ncsp_en.reldate:6:r, ncsp_en.subst:6:r, code_den, code_fin, code_eng, code_nor, code_swe,code_ice save nomodify nowait
return