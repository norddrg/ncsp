...
do while not eof()
 ...
 if found() and reldate<> ctod(space(8))
   ...
   if found()
     if ncsp_en.code_lat=' '
       ...
       append blank
       replace code with ncsp_koo.ncsp, code_lat with ncsp_koo.code, released with .f., text with ncsp_koo.text, usedate with ncsp_koo.usedate, chadate with date(), nc with .f. 
 ...
 if not found() or reldate <> ctod(space(8))
  ...
  if ncsp_koo.ncsp<>' '
    ...
    if found() 
      if code_lat=' ' or reldate <> ctod(space(8))
        replace code_lat with ncsp_koo.code
  ...
  if not lc_found
    select ncsp_en
    append blank
    replace code with ncsp_koo.code, code_lat with ncsp_koo.code, released with .f., text with ncsp_koo.text, usedate with ncsp_koo.usedate, chadate with date(), nc with .f. 
