...
SET SAFETY OFF 
SELECT ncsp_luv
COPY TO ..\text\swe_ncsp_luv.txt SDF 
SELECT ncsp_ryh
COPY TO ..\text\swe_ncsp_ryh.txt SDF 
SELECT ncsp_ala
COPY TO ..\text\swe_ncsp_ala.txt SDF 
SELECT ncsp_koo
COPY TO ..\text\swe_ncsp_koo.txt SDF 
SET SAFETY ON 

define popup suodval from 5,5 font  max_foty,  max_fosi prompt files like s_*.dbf
on selection popup suodval do ..\ruotsi\suodval with prompt()
