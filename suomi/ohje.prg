...
*endif
  ...
  on key label f12 do ..\suomi\kasit
*ON KEY LABEL ctrl+f12 DO ..\suomi\lisays
*  on key label alt+f12 do ..\suomi\pvmmuu
  on key label f11 do ..\suomi\paiv
  on key label ctrl+f11 do ..\suomi\suodatin
  on key label alt+f11 do ..\suomi\lang_pal
  on key label f9 do ..\suomi\suodkay
  on key label alt+f9 do ..\suomi\suodval1
  on key label f8 do ..\suomi\ventncsp
  on key  label ctrl+f8 do  ..\suomi\siirmuun
  on key label alt+f8 do ..\suomi\siir_drg
  on key label f7 do ..\suomi\simupda
  on key label f6 do ..\suomi\aakalo
  on key label ctrl+f6 do ..\suomi\aakkos

*  on key label f5 do ..\suomi\vanhat

*  on key label f2 do ..\suomi\valinta
*  on key label f3 do \data\icd_10\icdnayt\_icdnay
  on key label alt+f do ..\suomi\koodets
  on key label alt+b do ..\suomi\lisays
  on key label  ctrl+b do ..\suomi\isonay
*  on key label alt+f11 do ..\suomi\englanti
  on key label ctrl+s do ..\suomi\suomi
  on key label ctrl+r do ..\suomi\ruotsi
  on key label f10

  on key label alt+f1 do ..\suomi\ncsp
  on key label ctrl+f1 do lopetus
  on key label alt+l do ..\suomi\lang_pal
return
