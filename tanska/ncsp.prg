procedure ncsp
  ...
  set talk off
  set near on
  public  luv_y, luv_x, ryh_y, ala_y, kieli, pub_code, laji, aktfilt, suodatin
  kieli='S'
  lc_loop=.t.
  laji='A'
  suodatin=.f.
...
SET SAFETY OFF 
SELECT ncsp_luv
COPY TO ..\text\den_ncsp_luv.txt SDF 
SELECT ncsp_ryh
COPY TO ..\text\den_ncsp_ryh.txt SDF 
SELECT ncsp_ala
COPY TO ..\text\den_ncsp_ala.txt SDF 
SELECT ncsp_koo
...
on selection popup suodval do ../tanska/suodval with prompt()
