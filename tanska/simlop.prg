...
do while sl_loop
  ...
  if sl_kier=1
    sl_code=trim(code_den)
    select ncsp_koo
    seek sl_code
    if not found()
      select ncsp_koo
      browse window koodit fields ncsp_koo.released:2:r, ncsp_koo.code:10, ncsp_koo.text:50, ncsp_koo.short:20,;
      ncsp_koo.ncsp:9, ncsp_koo.chadate:6 :r, ncsp_koo.usedate:6:r, ;
      ncsp_koo.reldate:6:r, ncsp_koo.english:50 nowait save nomodify
      select ncsp_en
      set order to code_den
      browse window codes fields ncsp_en.ncsp:9, ncsp_en.nc:2, ncsp_en.code_den:10, ncsp_en.text:50,;
      ncsp_en.chadate:6 :r, ncsp_en.usedate:6:r, ;
      ncsp_en.reldate:6:r, ncsp_en.subst:6:r, code_nor, code_swe, code_eng, code_est, code_fin save nomodify nowait
      wait window sl_code +' local code not found in local version - remove the code from combined table? Y/N'
  ...
  if sl_kier=2
    ...
    skip
    if code_den=sl_code and sl_code=code_den
    set step on
      select ncsp_koo
      ...
      select ncsp_en
      set order to code
      browse window codes fields ncsp_en.ncsp:9, ncsp_en.nc:2, ncsp_en.code_den:7, ncsp_en.text:50,;
      ncsp_en.chadate:6 :r, ncsp_en.usedate:6:r, ;
      ncsp_en.reldate:6:r, ncsp_en.subst:6:r, code_nor, code_swe, code_eng, code_est, code_fin save nomodify nowait
      wait window 'Local code not found in local version - delete from combined table? Y/N'
      if lastkey()=89 or lastkey()=121 
