 ...
 if found() and reldate<> ctod(space(8))
   ...
   if found()
     if ncsp_en.code_den=' '
       ...
       append blank
       replace code with ncsp_koo.ncsp, code_den with ncsp_koo.code, released with .f., text with ncsp_koo.english, usedate with ncsp_koo.usedate, chadate with date(), nc with .f. 
       if text=' ' 
 ...
 *  seek sim_code
 ...
 if not found() or reldate <> ctod(space(8))
  ...
  if ncsp_koo.ncsp<>' '
    ...
    if found() 
      if code_den=' ' or reldate <> ctod(space(8))
  ...
  if not lc_found
    select ncsp_en
    append blank
    replace code with ncsp_koo.code, code_den with ncsp_koo.code, released with .f., text with ncsp_koo.english, usedate with ncsp_koo.usedate, chadate with date(), nc with .f. 
    if text=' ' 
      replace text with ncsp_koo.text
    endif
